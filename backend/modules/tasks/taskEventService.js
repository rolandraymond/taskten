...
/**
 * 🚀 الحصول على الخط الزمني التفصيلي ليوم اليوزر مع حساب نقاط الـ KPI
 */
const getUserDailyActivityFeed = async (userId, dateStr) => {
    const { Op } = require('sequelize');
    const { Task } = require('../../models');

    // 1. تحديد بداية ونهاية اليوم (Time range)
    const startDate = new Date(dateStr);
    startDate.setHours(0, 0, 0, 0);
    const endDate = new Date(dateStr);
    endDate.setHours(23, 59, 59, 999);

    // 2. جلب كل الأحداث لليوزر ده في اليوم ده (مع اسم التاسك)
    const events = await TaskEvent.findAll({
        where: {
            user_id: userId,
            created_at: {
        ...
        include: [
            {
                model: Task,
                as: 'Task',
                attributes: ['id', 'uid', 'name', 'parent_task_id'], // ✅ التعديل هنا: جلب parent_task_id
                include: [
                    {
                        model: Task,
                        as: 'ParentTask', // ✅ التعديل هنا: جلب الـ UID الخاص بالمهمة الأساسية
                        attributes: ['uid'],
                        required: false,
                    },
                ],
        ...
        order: [['created_at', 'DESC']], // من الأحدث للأقدم
    });

    // 3. نظام الأوزان (KPI Scoring System) والترجمة البشرية
    let totalScore = 0;
    const activityFeed = events.map((event) => {
        let actionText = '';
        let points = 0;
        let iconType = 'info';
        ...
        switch (event.event_type) {
            case 'created':
                actionText = 'قام بإنشاء مهمة جديدة';
                points = 2;
                iconType = 'plus';
                break;
            case 'status_changed':
                ...
                if (newStatus === 1) {
                    actionText = 'بدأ العمل على المهمة (In Progress)';
                    points = 3;
                    iconType = 'play';
                } else if (newStatus === 2) {
                    actionText = 'أنهى المهمة بنجاح (Completed) 🎉';
                    points = 10; // أعلى نقاط للإنجاز
                    iconType = 'check';
                } else {
                    actionText = 'قام بتغيير حالة المهمة';
                    points = 1;
                }
                break;
            case 'note_changed':
                actionText = 'أضاف أو عدل ملاحظة في المهمة';
                points = 1;
                iconType = 'edit';
                break;
            case 'due_date_changed':
                actionText = 'قام بتعديل تاريخ استحقاق المهمة';
                points = 1;
                iconType = 'calendar';
                break;
            case 'priority_changed':
                actionText = 'قام بتعديل أولوية المهمة';
                points = 1;
                iconType = 'flag';
                break;
            case 'completed': // في حالة استخدام هذا الـ Event مباشرة
                actionText = 'أكمل المهمة';
                points = 10;
                iconType = 'check';
                break;
            case 'completion_based_changed': // أو لو مسمينها checklists_changed
                actionText = 'قام بتحديث المهام الفرعية (Subtasks)';
                points = 2; // نقطتين على كل تحديث لساب-تاسك
                iconType = 'list';
                break;
            case 'description_changed': // لو بتكتبوا الساب تاسكس جوه الوصف
                actionText = 'قام بتحديث تفاصيل المهمة / المهام الفرعية';
                points = 1.5;
                iconType = 'edit';
                break;
            default:
                actionText = `قام بإجراء تعديل (${event.event_type})`;
                points = 0.5;
                iconType = 'update';
        }
        ...
        return {
            ...
            taskName: event.Task ? event.Task.name : 'مهمة محذوفة',
            taskUid: event.Task ? event.Task.uid : null,

            // ✅ التعديل هنا: تحديد ما إذا كانت المهمة فرعية وإرسال uid الأب إن وجد
            isSubtask: !!(event.Task && event.Task.parent_task_id),
            parentTaskUid:
                event.Task && event.Task.ParentTask
                    ? event.Task.ParentTask.uid
                    : null,

            actionText,
            points,
            iconType,
            details: { old: event.old_value, new: event.new_value }, // للأدمن لو حب يشوف تفاصيل تقنية
        };
    });
