const { Task, Tag, Project, User, sequelize } = require('../../../models'); // ضفنا User هنا
...
async function filterTasksByParams(
    ...
    let includeClause = [
        // 👇 ضيف البلوك ده كأول عنصر في الـ includeClause 👇
        {
            model: User,
            as: 'Assignees',
            attributes: ['id', 'name', 'surname', 'email', 'avatar_image'],
            through: { attributes: [] },
            required: false, // مهمة جداً عشان متخفيش التاسكات اللي ملهاش Assignees
        },
        // 👆 ------------------------------------------- 👆
        {
            model: Tag,
        ...
        [Op.and]: [
            {
                [Op.or]: [
                    ownedOrShared,
                    { '$Assignees.id$': userId }, // السطر السحري اللي هيخلي اليوزر يشوف التاسك لو اتعمله Assign ليها
                ],
            },
            whereClause,
        ],
...
function getTaskIncludeConfig() {
    return [
        // 👇 الإضافة الجديدة عشان تفاصيل التاسك تقرأ الـ Assignees 👇
        {
            model: User,
            as: 'Assignees',
            attributes: ['id', 'name', 'surname', 'email', 'avatar_image'],
...
function getTaskIncludeConfigLight() {
    return [
        // 👇 الإضافة هنا كمان للنسخة الخفيفة 👇
        {
            model: User,
            as: 'Assignees',
            attributes: ['id', 'name', 'surname', 'email', 'avatar_image'],
