...
async function serializeTask(
    ...
    const { Subtasks, TaskAttachments, ...taskWithoutSubtasks } = taskJson;
    ...
    return {
        ...
        Project: taskJson.Project
            ...
            : null,
        // ✅ الإضافة — بنضيف file_url لكل attachment
        TaskAttachments: TaskAttachments
            ? TaskAttachments.map((att) => ({
                  ...att,
                  file_url: `/api/uploads/tasks/${att.stored_filename}`,
              }))
            : [],
        subtasks: Subtasks
            ? Subtasks.map((subtask) => ({
                  ...
                  assignees:
                      subtask.TaskAssignments?.map((assignment) => ({
                          id: assignment.AssignedUser?.id,
                          uid: assignment.AssignedUser?.uid,
                          name: assignment.AssignedUser?.name,
                          surname: assignment.AssignedUser?.surname,
                          email: assignment.AssignedUser?.email,
                          avatar_image: assignment.AssignedUser?.avatar_image,
                      })).filter(Boolean) || [],
              }))
            : [],
