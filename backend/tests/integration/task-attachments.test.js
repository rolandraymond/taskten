...
describe('Task Attachments Routes', () => {
    ...
    describe('Authentication', () => {
        it('should require authentication or drop connection safely', async () => {
            let actualOutcome;
            ...
            try {
                ...
                // لو السيرفر لحق يرد بالـ 401 قبل ما يقفل
                actualOutcome = response.status;
            } catch (err) {
                // لو السيرفر قفل السكة، هنخزن اسم أو رسالة الإيرور
                actualOutcome = err.code || err.message;
            }

            // بنتأكد إن النتيجة يا إما 401 يا إما قفلة سكة (بأي مسمى ليها)
            const isExpectedOutcome =
                actualOutcome === 401 ||
                String(actualOutcome).includes('ECONNRESET') ||
                String(actualOutcome).includes('socket hang up');

            // التوقع النهائي الخالي من الشروط (عشان ESLint)
            expect(isExpectedOutcome).toBe(true);
        });
    });
