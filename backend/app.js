...
// ====================================================
// ✅ Single Source of Truth for Uploads (Scalable & Secure)
// ====================================================
...
const staticFileOptions = {
    setHeaders: (res, path, stat) => {
        ...
        // تأكيد إضافي للسماح للفرانتد بقراءة الملف
        res.set('Access-Control-Allow-Origin', '*');
    },
};

// تطبيق الإعدادات على كل مسارات الـ Uploads
app.use('/api/uploads', express.static(uploadsPath, staticFileOptions));
app.use('/uploads', express.static(uploadsPath, staticFileOptions));

if (API_VERSION && API_BASE_PATH !== '/api') {
    app.use(
        `${API_BASE_PATH}/uploads`,
        express.static(uploadsPath, staticFileOptions)
    );
}
// ====================================================
...
async function startServer() {
    try {
        ...
        // ✅ ضيف السطرين دول هنا
        const {
            registerTaskNotificationListeners,
        } = require('./modules/notifications/taskEventListeners');
        registerTaskNotificationListeners();
        // ────────────────────────────────
