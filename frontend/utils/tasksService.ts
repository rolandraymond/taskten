...
export const assignTask = async (taskUid: string, userId: number) => {
    const response = await fetch(
        getApiPath(`task/${encodeURIComponent(taskUid)}/assign`),
        {
            method: 'POST',
            credentials: 'include',
            headers: await getPostHeadersWithCsrf(),
    ...
    await handleAuthResponse(response, 'Failed to assign task.');
    return await response.json();
};

export const unassignTask = async (taskUid: string, userId: number) => {
    const response = await fetch(
        getApiPath(`task/${encodeURIComponent(taskUid)}/assign`),
        {
            method: 'DELETE',
    ...
    await handleAuthResponse(response, 'Failed to unassign task.');
    return await response.json();
};
// 👆👆 Admin Assignment Functions 👆👆
export const updateTaskAssignments = async (taskUid: string, userIds: number[]): Promise<Task> => {
    const response = await fetch(getApiPath(`task/${encodeURIComponent(taskUid)}/assign-multiple`), {
        method: 'PUT',
        credentials: 'include',
        headers: await getPostHeadersWithCsrf(),
        body: JSON.stringify({ user_ids: userIds }),
    });
    await handleAuthResponse(response, 'Failed to update assignments.');
    return await response.json();
};
export const assignSubtask = async (
    ...
    const response = await fetch(
        getApiPath(`task/${encodeURIComponent(subtaskUid)}/assign-multiple`),
        {
            method: 'PUT',
            credentials: 'include',
            headers: await getPostHeadersWithCsrf(),
    ...
    await handleAuthResponse(response, 'Failed to update subtask assignments.');
