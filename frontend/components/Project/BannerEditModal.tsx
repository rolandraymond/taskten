...
import { fetchWithCsrf } from '../../utils/csrfService'; // اتأكد إن المسار ده صح حسب الفولدرات عندك
...
const BannerEditModal: React.FC<BannerEditModalProps> = ({
    ...
    const handleImageUpload = async (): Promise<string | null> => {
        ...
        try {
            ...
            // 👇 استخدمنا fetchWithCsrf بدل fetch
            const response = await fetchWithCsrf(
                getApiPath('upload/project-image'),
                {
                    method: 'POST',
                    credentials: 'include',
                    body: formData,
                    // متكتبش Content-Type خالص هنا عشان المتصفح يحط الـ boundary بتاع الصورة أوتوماتيك
                }
            );
