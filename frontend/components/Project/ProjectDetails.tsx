...
const ProjectDetails: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadProjectData = async () => {
            try {
                ...
                setError({ isError: false }); // Reset error state before loading
   ...
   if (error.isError) {
        // 🔒 لو الخطأ 403 (ليس لديه صلاحية)، اعرض الكارت الأنيق
        if (error.status === 403) {
            return (
                <div className="flex items-center justify-center min-h-[75vh] w-full bg-transparent">
                    <div className="text-center p-8 max-w-md bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                        <div className="text-6xl mb-6 select-none">🔒</div>
                        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-3">
                            Limited Access
                        </h2>
                        <p className="text-gray-500 dark:text-gray-400 mb-8 text-sm leading-relaxed">
                            You have access to specific tasks within this project, but you do not have permission to view the full project dashboard.
                        </p>
                        <button
                            onClick={() => navigate(-1)}
                            className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                        >
                            Go Back
                        </button>
                    </div>
        ...
        // ⚠️ لو إيرور عادي (مثلاً السيرفر واقع)، اعرض الرسالة دي
        return (
            <div className="flex items-center justify-center h-screen bg-gray-50 dark:bg-gray-900">
                <div className="text-red-600 font-medium bg-red-50 dark:bg-red-900/20 px-6 py-3 rounded-lg border border-red-100 dark:border-red-800">
