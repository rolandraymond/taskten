...
import TaskAssignmentsCard from './TaskDetails/TaskAssignmentsCard';
import {
    updateTask,
    deleteTask,
    fetchTaskByUid,
...
const TaskDetails: React.FC = () => {
    ...
    const usersStore = useStore((state: any) => state.usersStore);
    ...
    const recurrenceWeekdaysStr = JSON.stringify(
    ...
    }, [tagsStore.hasLoaded, tagsStore.isLoading]); // 👈 Avoid full store dependency
    ...
    // 👇 Loop Killer 1: Removed 'task' and 'tasksStore' objects from dependency array, only use 'task?.uid'
    useEffect(() => {
    ...
    // 👇 Loop Killer 2: Stringify the subtasks array so React doesn't re-run on new object references
    const subtasksDependencyStr = JSON.stringify(task?.subtasks || []);
    ...
    useEffect(() => {
        ...
        loadSubtasks();
    }, [activePill, task?.uid, subtasksDependencyStr, hasLoadedSubtasks]); // 👈 Only safe primitives!

    // 👇 Loop Killer 3: Use the stringified dependency instead of array reference
    useEffect(() => {
        setPendingSubtasks(task?.subtasks || []);
    }, [subtasksDependencyStr]);
        ...
        <div className="px-4 lg:px-6 pt-4">
            <div className="w-full">
                <TaskDetailsHeader
                    ...
                    // ✅ السطر ده هيخفي زرار السلة لو هو مش أدمن
                    ...
                    {activePill === 'overview' && (
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                            ...
                            <div className="space-y-6">
                                {/* 👇 كارت تكليف الأدمن 👇 */}
                                {/* ✅ Admin Assign Card */}
                                {/* ✅ Premium Task Assignments Card */}
                                <TaskAssignmentsCard
                                    task={task}
                                    /* allUsers={usersStore.users} */
                                    onRefresh={() =>
                                        uid && tasksStore.loadTaskByUid(uid)
                                    }
                                />
                                ...
                                <TaskProjectCard
                                    ...
                                    // لو عايز تمنعه يغير البروجيكت كمان:
                                    onProjectSelect={
                                        isUserAdmin
                                            ? handleProjectSelection
                                            : undefined
                                    }
                                    // ✅ لو عايز تمنعه يمسح البروجيكت من التاسك:
                                    onProjectClear={
                                        isUserAdmin
                                            ? handleClearProject
                                            : undefined
                    ...
                    {activePill === 'subtasks' && (
                        <div className="grid grid-cols-1">
                            <TaskSubtasksCard
                                ...
                                // أضف هذه الدالة هنا لتحديث الـ Store فوراً
                                onSubtaskUpdate={async (updatedSubtask) => {
                                    // تحديث المهمة الفرعية في الـ State المحلي
                                    const updatedPending = pendingSubtasks.map(
                                        (st) =>
                                            st.id === updatedSubtask.id
                                                ? updatedSubtask
                                                : st
                                    );
                                    setPendingSubtasks(updatedPending);

                                    // تحديث الـ tasksStore بالكامل لضمان تزامن بيانات المهمة الرئيسية مع الفرعية
                                    const updatedTasks = tasksStore.tasks.map(
                                        (t: Task) =>
                                            t.uid === task.uid
                                                ? {
                                                      ...t,
                                                      subtasks: updatedPending,
                                                  }
                                                : t
                                    );
                                    tasksStore.setTasks(updatedTasks);
