...
interface StoreState {
    ...
    usersStore: UsersStore; // 2. أضف الـ Store
}
...
export const useStore = create<StoreState>((set: any) => ({
    ...
    // 3. أضف الـ Implementation جوه الـ create
    usersStore: {
        users: [],
        isLoading: false,
        loadUsers: async () => {
    const state = useStore.getState();
    if (state.usersStore.isLoading) return;

    const { getAssignmentPicker } = await import('../utils/usersService');

    set((state) => ({
        usersStore: { ...state.usersStore, isLoading: true },
    }));

    try {
        const users = await getAssignmentPicker();
        set((state) => ({
            usersStore: {
                ...state.usersStore,
                users,
        ...
        set((state) => ({
            usersStore: { ...state.usersStore, isLoading: false },
        }));
    }
},
        // 👇 الأكشن الجديد للترقية
        changeUserRole: async (userId: number, newRole: string) => {
            const { updateUserRole } = await import('../utils/usersService');
            try {
                await updateUserRole(userId, newRole);
                
                // تحديث الستيت أوتوماتيك عشان الـ UI يتحدث فوراً
                set((state: any) => ({
                    usersStore: {
                        ...state.usersStore,
                        users: state.usersStore.users.map((u: any) =>
                            u.id === userId 
